...
from image_module.split import load_gray, split_patches
from image_module.extractor import extract_patch
from image_module.codebook import load_codebook, assign
...
if __name__ == "__main__":
    ...
    sample = sys.argv[1] if len(sys.argv) > 1 else next(
        (str(p) for p in Path("data/raw/images").glob("*.jpg")), None
    )
    if not sample:
